class EnemiesInGame {
        ...
        for (let j = 0; j < this.enemies[m][i].magazine.length; j++) {
          ...
          if (
            choosePlayer.triPointHitbox(
              ...
              this.enemies[m][i].magazine[j].y + this.enemies[m][i].magazine[j].height / 2
            )
          ) {
      ...
      for (let i = 0; i < this.enemies[m].length; i++) {
        ...
        for (let l = 0; l < this.enemies[m][i].magazine.length; l++) {
          ...
          for (let j = 0; j < choosePlayer.magazine.length; j++) {
            ...
            if (
              playerBullet.y - playerBullet.height / 2 <= enemyBullet.y + enemyBullet.height / 2 &&
  ...
  reachedSurface(choosePlayer) {
    for (let m = 0; m < this.enemies.length; m++) {
      for (let i = 0; i < this.enemies[m].length; i++) {
        if (this.enemies[m][i].y + this.enemies[m][i].height / 2 >= choosePlayer.y - choosePlayer.height / 2) {
          choosePlayer.hp = 0
          break
        }
      }
...
class UfoInGame {
  ...
  damagedByPlayer(choosePlayer) {
    for (let j = 0; j < choosePlayer.magazine.length; j++) {
      ...
      //if bullet hits enemy (compares enemy and bullet location)
      if (
        playerBullet.y - playerBullet.height / 2 <= this.ufo.y + this.ufo.height / 2 && //if player bullet at enemy altitude
