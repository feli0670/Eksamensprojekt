  ...
  update(choosePlayer, gamestate) {
    for (let j = 0; j < this.enemies.length; j++) {
      ...
      for (let i = 0; i < this.enemies[j].length; i++) {
        ...
        } else if (this.deadEnemies == 0) {
          //else if all enemies are died (deadEnemies reset)
    ...
    for (let m = 0; m < this.enemies.length; m++) {
      ...
      for (let i = 0; i < this.enemies[m].length; i++) {
        ...
        for (let j = 0; j < this.enemies[m][i].magazine.length; j++) {
          ...
          if (choosePlayer.triPointHitbox(choosePlayer.x1, choosePlayer.y1, choosePlayer.x2, choosePlayer.y2, choosePlayer.x3, choosePlayer.y3, this.enemies[m][i].magazine[j].x, this.enemies[m][i].magazine[j].y + this.enemies[m][i].magazine[j].height / 2)) {
      ...
      for (let i = 0; i < this.enemies[m].length; i++) {
        ...
        for (let j = 0; j < choosePlayer.magazine.length; j++) {
          ...
          if (compareEnemy.alive) {
            if (
              playerBullet.y - playerBullet.height / 2 <= compareEnemy.y + compareEnemy.height / 2 && //if player bullet at enemy altitude
              playerBullet.x + playerBullet.width / 2 >= compareEnemy.x - compareEnemy.width / 2 && //if player bullet on right side of bunker edge
              playerBullet.x - playerBullet.width / 2 <= compareEnemy.x + compareEnemy.width / 2
            ) {
              //if player bullet on left side of enemy edge
      ...
      for (let i = 0; i < this.enemies[m].length; i++) {
        ...
        for (let l = 0; l < this.enemies[m][i].magazine.length; l++) {
          ...
          for (let j = 0; j < choosePlayer.magazine.length; j++) {
            ...
            if (playerBullet.y - playerBullet.height / 2 <= enemyBullet.y + enemyBullet.height / 2 && playerBullet.x + playerBullet.width / 2 >= enemyBullet.x - enemyBullet.width / 2 && playerBullet.x - playerBullet.width / 2 <= enemyBullet.x + enemyBullet.width / 2) {
...
class BunkersInGame {
  ...
  damagedByPlayer(choosePlayer) {
    for (let k = 0; k < this.bunkers.length; k++) {
      ...
      for (let j = 0; j < choosePlayer.magazine.length; j++) {
        ...
        if (
          playerBullet.y - playerBullet.height / 2 <= compareBunkers.y + compareBunkers.height / 2 && //if player bullet at bunker altitude
          ...
          playerBullet.x - playerBullet.width / 2 <= compareBunkers.x + compareBunkers.width / 2
        ) {
          //if player bullet on left side of enemy edge
  ...
  damagedByEnemy(chooseEnemies) {
    for (let m = 0; m < chooseEnemies.enemies.length; m++) {
      //runs through every enemy
      for (let i = 0; i < chooseEnemies.enemies[m].length; i++) {
        //runs through every enemy
        for (let j = 0; j < chooseEnemies.enemies[m][i].magazine.length; j++) {
          //runs through enemy magazine and shoots the bullets once loaded into magazine, also assures all bullets fired is still displayed
          for (let k = 0; k < this.bunkers.length; k++) {
            //runs through every bunker
            ...
            if (
              enemyBullet.y >= compareBunkers.y - compareBunkers.height / 2 && //if enemy bullet at bunker altitude
              enemyBullet.x + enemyBullet.width / 2 >= compareBunkers.x - compareBunkers.width / 2 && //if enemy bullet on right side of bunker edge
              enemyBullet.x - enemyBullet.width / 2 <= compareBunkers.x + compareBunkers.width / 2
            ) {
              //if enemy bullet on left side of enemy edge
